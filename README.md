...
- 注：卡片渲染**只在阅读模式生效**。Live Preview 下嵌入保持 Obsidian 原生样式（这是 Obsidian 编辑器的渲染路径限制，插件无法接管），按 `Ctrl/Cmd+E` 切到阅读模式即变卡片。
...
- **从文件列表把笔记拖进编辑器** → 生成 `![[ ]]` 嵌入并直接渲染成卡片。
  Obsidian 原生拖文件得到的是 `[[ ]]` 链接，插件会把它改写成嵌入。
  不想要这个行为，可在设置里关掉「拖入笔记时插入嵌入 ![[ ]]」。
- **从卡片头部拖到正文任意位置** → 插入该笔记的嵌入。
  只有头部可拖，正文区留给选中复制和折叠点击。
- **把卡片拖到另一张卡片上** → 调整顺序：悬停在上半部插到它前面，下半部插到后面（有指示线）。
  实际改的是源码里 `![[ ]]` 行的顺序。
  只有**同级**卡片之间能排 —— 拖嵌套的小卡片不会动到外层的大卡片。
